...
function defaultLayout(slug: string): LayoutJson {
  ...
  // Modelos novos iniciam 100% LIMPOS (sem componentes pré-adicionados)
  return {};
}
...
export function DesignerEditor({ template }: { template: InvitationTemplate }) {
  ...
  const [selected, setSelected] = useState<FieldKey | null>(() => {
    const keys = orderedKeys(initialJson);
  ...
  function clearAllFields() {
    const total = orderedKeys(layout).length;
    if (total === 0) return;
    if (window.confirm(`Tem a certeza que deseja remover todos os ${total} componentes do convite?`)) {
      setLayout({});
      setSelected(null);
    }
  }
  ...
  function resetLayout() {
    const fresh = defaultLayout(template.slug);
    setLayout(fresh);
    const keys = orderedKeys(fresh);
  ...
  const selectedField = selected ? (layout[selected as string] as LayoutField | undefined) : undefined;
  const frameStyle = {
    width: CANVAS.width,
    ...
    <main className="flex h-screen flex-col bg-gray-100">
      <header className="flex items-center justify-between border-b border-gray-200 bg-white px-6 py-3">
        ...
        <div className="flex items-center gap-3">
          {orderedKeys(layout).length > 0 && (
            <button
              onClick={clearAllFields}
              title="Remover todos os componentes e começar com o modelo em branco"
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-80 shrink-0 overflow-y-auto border-r border-gray-200 bg-white p-4">
          ...
          {(() => {
            ...
            return (
              <div className="mb-3 rounded-lg border border-dashed border-rose-300 p-2.5 bg-rose-50/50">
                <label className="block text-xs font-semibold text-rose-800 mb-1">
                  + Adicionar componente ao convite:
                </label>
                <select
                  className="w-full rounded-md border border-gray-300 bg-white px-2 py-1.5 text-xs text-gray-800 shadow-sm focus:border-rose-500 focus:outline-none focus:ring-1 focus:ring-rose-500 font-medium"
                  defaultValue=""
                  onChange={(e) => {
                    if (e.target.value) {
                ...
                >
                  <option value="" disabled>
                    Escolha um componente para adicionar...
                  </option>
          ...
          {!selectedField || !selected ? (
            <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50/80 p-5 text-center text-gray-500">
              <p className="text-sm font-semibold text-gray-700">Nenhum componente selecionado</p>
              <p className="mt-1 text-xs text-gray-500 leading-relaxed">
                {orderedKeys(layout).length === 0
                  ? "O modelo está limpo. Utilize o seletor acima para adicionar apenas os componentes que desejar ao convite."
                  : "Clique num dos componentes ativos na lista acima para ajustar posição, fonte e texto."}
              </p>
            </div>
          ) : (
            <div>
